...
### Implemented
- [x] Rorschach Structural Summary calculator (legacy GAS logic, 1:1 match — v1↔v2 cross-verified)
...
- [x] Reference page (`/ref`) with sub-tab layout: Document Search / Skill Book Store (Coming Soon)

### Not Yet Implemented
- [ ] **Rorschach-specific system prompt** (core interpretation guidelines) ← **NEXT PRIORITY**
- [ ] Skill Book data model & management
- [ ] Skill Book Store (marketplace)
- [ ] Skill Book Builder (AI-assisted creation tool)
...
User (Clinical Psychologist)
  │
  ├─ Inputs Rorschach responses → Calculator → Structural Summary
  │
  ├─ Opens AI Chat
  │     │
  │     ├─ Base System Prompt (our core, hidden from users)
  │     ├─ + Active Skill Book (instructions + documents)
  │     ├─ + Structural Summary results (auto-injected)
  │     ├─ + User's question
  │     │
  │     └─ → LLM API (user's own key) → Streaming response
  │
  ├─ Skill Book Store
  │     ├─ Browse & purchase Skill Books (credits)
  │     ├─ Publish own Skill Books (listing fee)
  │     └─ Earn credits from sales (minus commission)
  │
  └─ Skill Book Builder
        ├─ Upload raw materials (textbooks, notes as PDF/TXT)
        ├─ AI converts to structured Skill Book format
        └─ Costs credits (uses platform's API key, not user's)
```
...
- [ ] Write comprehensive base system prompt for Rorschach interpretation
- [ ] Include: interpretation principles, cluster analysis rules, index calculations
- [ ] Store in `/lib/systemPrompts/` (not user-accessible, but version-controlled)
- [ ] Multi-language awareness in prompt (handle 5 supported languages)
...
- [x] Loading UX: "검사 결과를 읽는 중입니다..." → AI processes → "결과 준비됨"

### 1.3 Skill Book Foundation (Data Model Only)
- [ ] Define Prisma schema for SkillBook model
- [ ] Basic CRUD API routes
- [ ] Ship 1 default built-in Skill Book (our own Rorschach interpretation guide)
- [ ] Allow users to select active Skill Book in chat
...
- [ ] Create/edit/delete own Skill Books
- [ ] Preview how a Skill Book affects AI behavior
- [ ] Import/export Skill Book as JSON
